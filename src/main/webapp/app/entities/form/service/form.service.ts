...
export class FormService {
  ...
  update(form: IForm): Observable<EntityResponseType> {
    return this.http.put<IForm>(`${this.resourceUrl}/${getFormIdentifier(form) as string}`, form, { observe: 'response' });
  }

  partialUpdate(form: IForm): Observable<EntityResponseType> {
    return this.http.patch<IForm>(`${this.resourceUrl}/${getFormIdentifier(form) as string}`, form, { observe: 'response' });
  }
